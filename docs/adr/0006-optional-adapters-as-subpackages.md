...
- Adapters live in **subpackages of the same module**, not separate modules:
  ...
  `extract/html`, `extract/pdf`, `extract/docx`, `extract/pptx`,
  `extract/markdown`.
- Root packages stay dependency-free: `store` is the byte interface only;
  ...
  `golang.org/x/net/html`, which is not the standard library. (DOCX, PPTX, and
  Markdown are stdlib-only — DOCX/PPTX via `archive/zip` + `encoding/xml` — but
  stay subpackages anyway, so all formats are uniformly opt-in.)
...
- A golangci-lint depguard `core-no-adapters` rule forbids core packages
  ...
  subpackages (`extract/html|pdf|docx|pptx|markdown`, `store/gcs`, `index/*`). It
