...
| Package    | In v1? | Shape |
|------------|:------:|-------|
| `extract`  | ✅ | Media-type-aware extraction: `Extractor` interface + registry + stdlib `text/plain` extractor in core. Registry canonicalizes media types, bounds input size (`WithMaxBytes`), and rejects empty `parentID`. Returns `[]content.Artifact` (multi-modal; text-only today; `DerivedFrom`=parentID, ID left for the caller). Every non-plain-text format is an opt-in subpackage (ADR 0006), enforced by a depguard rule: `extract/html` (golang.org/x/net/html), `extract/pdf` (pluggable engines, no unsafe default — `extract/pdf/pdftotext` out-of-process Poppler [recommended, per-page artifacts] and `extract/pdf/purego` dslipak trusted-only fallback, ADR 0010), `extract/docx` (stdlib zip/xml), `extract/pptx` (stdlib zip/xml; slide text + speaker notes, in presentation order via presentation.xml with a filename-order fallback), and `extract/markdown` (stdlib; verbatim, preserves structure for the chunker, ADR 0008) have landed. An `extract/preset` bundle wires them all up in one call. Lifted from Morris. |
...
- ADR 0007 — PDF watchdog stopgap (superseded by ADR 0010).
- ADR 0010 — Pluggable PDF engines; out-of-process pdftotext recommended, no unsafe default.
