...
| Package    | In v1? | Shape |
|------------|:------:|-------|
| `extract`  | ✅ | Media-type-aware extraction: `Extractor` interface + registry + stdlib `text/plain` extractor in core. Registry canonicalizes media types, bounds input size (`WithMaxBytes`), and rejects empty `parentID`. Returns `[]content.Artifact` (multi-modal; text-only today; `DerivedFrom`=parentID, ID left for the caller). Every non-plain-text format is an opt-in subpackage (ADR 0006), enforced by a depguard rule: `extract/html` (golang.org/x/net/html), `extract/pdf` (dslipak/pdf — ships with a timeout watchdog for a parser hang, ADR 0007), `extract/docx` (stdlib zip/xml), `extract/pptx` (stdlib zip/xml; slide text + speaker notes, in presentation order via presentation.xml with a filename-order fallback), and `extract/markdown` (stdlib; verbatim, preserves structure for the chunker, ADR 0008) have landed. An `extract/preset` bundle wires them all up in one call. Lifted from Morris. |
